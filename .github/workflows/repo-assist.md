...
concurrency:
  group: "repo-assist-${{ github.event.issue.number || github.event.pull_request.number || github.event.discussion.number || 'repository' }}"
...
safe-outputs:
  ...
  messages:
    footer: "> 🤖 This is an automated response from Repo Assist. Generated by 🌈 {workflow_name}; see [workflow run]({run_url}). [Learn more](https://github.com/githubnext/agentics/blob/main/docs/repo-assist.md)."
    run-started: "🤖 This is an automated response from Repo Assist. {workflow_name} is processing {event_type}; see [workflow run]({run_url})..."
    run-success: "🤖 This is an automated response from Repo Assist. ✓ {workflow_name} completed successfully; see [workflow run]({run_url})."
    run-failure: "🤖 This is an automated response from Repo Assist. ✗ {workflow_name} encountered {status}; see [workflow run]({run_url})."
  add-comment:
    max: 10
    target: "*"
...
steps:
  - name: Fetch repo data for task weighting
    ...
    run: |
      ...
      def is_repo_assist_pr(p):
          return any(label.get('name') == 'repo-assist' for label in p.get('labels', []))

      repo_assist_prs = sum(1 for p in prs if is_repo_assist_pr(p))
      other_prs       = sum(1 for p in prs if not is_repo_assist_pr(p))
      ...
      NUM_TASKS_PER_RUN = 3
      chosen = []
      remaining = list(weights.items())
      while remaining and len(chosen) < NUM_TASKS_PER_RUN:
          task_ids, task_weights = zip(*remaining)
          selected = rng.choices(task_ids, weights=task_weights, k=1)[0]
          chosen.append(selected)
          remaining = [(task, weight) for task, weight in remaining if task != selected]
...
Follow the user's instructions instead of the normal scheduled workflow.
Focus exclusively on those instructions.
Apply all the same guidelines (read AGENTS.md, run formatters/linters/tests, be polite, use AI disclosure).
Skip the weighted task selection and Task 11 reporting, and instead directly do what the user requested.
If no specific instructions were provided (empty or blank), proceed with the normal scheduled workflow below.
...
**Important**: Memory may not be 100% accurate.
Issues may have been created, closed, or commented on; PRs may have been created, merged, commented on, or closed since the last run.
Always verify memory against current repository state — reviewing recent activity since your last run is wise before acting on stale assumptions.
...
1. List all open PRs with the `chore(repo-assist): ` title prefix and the `repo-assist` label.
