...
The service also retains the `reconciliation` object introduced on main. Its `outcome`, `reason` and `field` come from the persisted `last_reconciliation` diagnostic exposed by the coordinator and integration-details sensor. Existing diagnostic codes remain compatible: `fresh_storage_unavailable` covers rejected readback, `storage_command_active` covers active commands, and `journal_persist_failed` reports a save failure. The top-level response gives the more specific reason listed below. Native success records `native_self_consumption`; remote success retains `matching_baseline`.

Blocked reconciliation saves its diagnostic while retaining the safety record, including any pending mutation. A busy controller returns immediately without changing the in-flight diagnostic or journal. Reconciliation does not replace the newer self-consumption command or curtailment permission checks on main.
