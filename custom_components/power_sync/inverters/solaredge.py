...
class SolarEdgeEnergyController:
    ...
    async def reconcile_result(self) -> dict:
        ...
        async with session.lock:
            ...
            async def blocked(reason, fields=()):
                # Keep the published diagnostic codes while returning the more
                # specific readback failure to response clients.
                diagnostic_reason = {
                ...
                self._reconciliation_result(
                    session, "blocked", diagnostic_reason,
                    field=fields[0] if fields else None,
                )
                try:
                    await self._persist(
                        session, in_progress=session.pending_mutation is not None
                    )
                except Exception:
                    self._reconciliation_result(
            ...
            if observed is None:
                ...
                return await blocked(reason, fields)
            ...
            if not native:
                ...
                for key in ("storage_command_mode", "storage_default_mode"):
                    if observed.get(key) is None:
                        return await blocked("malformed_snapshot", (key,))
                    if _normalize_option(str(observed[key])) not in benign:
                        return await blocked("active_command", (key,))
            remote_fields = {
            ...
            if mismatches:
                return await blocked("baseline_mismatch", sorted(mismatches))
