...
def test_fleet_status_deduplicates_shared_site_grid_meter():
    """Each stack can report the same site CT, especially during force charge."""
    entry_1_states = _combined_states_for(
        1,
        battery_power="-4686",
        battery_soc="82.8",
    ...
    assert status["grid_power"] == pytest.approx(7.1505)
    assert status["battery_power"] == pytest.approx(-9.391)
    assert status["load_power"] == pytest.approx(1.2345)
...
def test_fleet_force_charge_prioritises_low_soc_stack_when_unbalanced():
    ...
    assert asyncio.run(controller.force_charge(duration_minutes=70, power_w=10000))

    assert controller.get_status()["surplus_balancer"]["status"] == "force_charging_low_stack"
    assert controller.get_status()["surplus_balancer"]["target_index"] == 0
    assert controller.get_status()["surplus_balancer"]["soc_parked_index"] == 1
    assert controller.get_status()["surplus_balancer"]["soc_parked_base_mode"] == "Normal"
    assert hass.services.calls == [
        ("number", "set_value", {"entity_id": "number.neovolt_1_dispatch_power", "value": 5.0}),
...
def test_fleet_force_charge_balancer_restores_parked_stack():
    entry_1_states = _combined_states_for(
        1,
        battery_power="0",
        battery_soc="48",
    ...
    assert controller.get_status()["surplus_balancer"]["soc_parked_base_mode"] is None
    assert hass.services.calls == [
...
def test_fleet_force_charge_refresh_preserves_restore_modes():
    entry_1_states = _combined_states_for(
        1,
        battery_power="0",
        battery_soc="48",
...
def test_fleet_surplus_balancer_parks_higher_soc_normal_stack():
    entry_1_states = _combined_states_for(
        1,
        battery_power="1000",
        battery_soc="40",
...
def test_fleet_surplus_balancer_parks_full_stack_until_lower_stack_catches_up():
    entry_1_states = _combined_states_for(
        1,
        battery_power="1424",
        battery_soc="100.0",
    ...
    assert balancer["status"] == "balancing_low_stack"
    assert balancer["lowest_soc_index"] == 1
    assert balancer["highest_soc_index"] == 0
    ...
    assert balancer["status"] == "soc_balanced"
    assert balancer["soc_parked_index"] is None
    assert hass.services.calls == [
        ("select", "select_option", {"entity_id": "select.neovolt_1_dispatch_mode", "option": "Normal"}),
    ]


def test_fleet_surplus_balancer_parks_charging_high_stack_when_low_stack_discharges():
    entry_1_states = _combined_states_for(
        1,
        battery_power="5060",
        battery_soc="92.4",
    ...
    entry_2_states = _combined_states_for(
        ...
        pv_power="0",
        grid_power="0",
    ) + _control_states_for(2, mode="Force Charge")
    hass = _FakeHass(
        entry_1_states + entry_2_states,
        {
            "neovolt-1": [state.entity_id for state in entry_1_states],
...
def test_fleet_surplus_balancer_restores_soc_parked_stack_when_balanced():
    entry_1_states = _combined_states_for(
        1,
        battery_power="1000",
        battery_soc="40",
