...
@pytest.mark.parametrize(
    ...
    [
        ...
        ("baseline", "native_self_consumption"),
    ],
)
...
def test_native_active_baseline_rejected_before_force_and_restore(command, timeout):
    hass = _SEHass()
    ...
    assert asyncio.run(controller.connect())
    # The containment controller rejects an already-active external baseline
    # before capture or any writes. Also reject a retained active snapshot.
    assert not asyncio.run(controller.force_charge(15, 2000))
    assert hass.services.calls == []
    controller._saved_control_state = {
        "storage_control_mode": "Remote Control",
        "storage_command_mode": command,
        "discharge_power_limit": "4200",
        "command_timeout": str(timeout),
    }
    assert controller._saved_control_state_contains_active_dispatch()
    assert not asyncio.run(controller.restore_normal())
    assert hass.services.calls == []
