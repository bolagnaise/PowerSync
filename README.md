...
| **Smart Optimization** | Built-in LP optimizer calculates optimal charge/discharge schedule using prices, solar, and load. It can optionally auto-apply the forecast-recommended software reserve floor and include planned EV load from a Home Assistant sensor for read-only or dumb EVSE setups. **Solar forecasting via Solcast or Open-Meteo Solar Forecast must be configured for accurate scheduling.** | [Details](https://github.com/bolagnaise/PowerSync/wiki/Smart-Optimization) |
